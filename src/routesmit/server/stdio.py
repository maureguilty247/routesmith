"""MCP-compatible stdio server for routesmit tool integration."""
...
from routesmit.state import list_routes, get_last_route
...
MCP_TOOLS = [
    {
        "name": "routesmit.run",
        "description": "Route and execute a prompt through the routesmit pipeline. Decomposes the prompt into capability-matched subtasks and executes them.",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    try:
        ...
        if method == "initialize":
            return _success(req_id, {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "routesmit", "version": "0.1.0"},
            })
...
def _handle_tool_call(
    ...
    elif tool_name == "routesmit.explain":
        executor = Executor(config=config)
    ...
    elif tool_name == "routesmit.detect_host":
        detection = detect_host(config)
        return _success(req_id, {
            "content": [{"type": "text", "text": json.dumps(detection.model_dump())}],
        })

    elif tool_name == "routesmit.capabilities":
        caps = get_host_capabilities(config)
        return _success(req_id, {
            "content": [{"type": "text", "text": json.dumps(caps.model_dump())}],
        })

    elif tool_name == "routesmit.history":
