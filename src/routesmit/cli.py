...
install_app = typer.Typer(help="Install routesmit configuration for a target host.")
...
def run(
    ...
    """Run a prompt through routesmit's host-aware routing pipeline."""
    config = load_config()
    if debug:
        config.debug = True
...
def detect_host_cmd() -> None:
    ...
    from routesmit.hosts.detector import detect_host
...
def capabilities() -> None:
    ...
    from routesmit.hosts.detector import get_host_capabilities
...
@app.command()
def doctor() -> None:
    """Run diagnostics on the routesmit environment."""
    import os
    import sys

    console.print(Panel("[bold]routesmit doctor[/bold]", border_style="blue"))
    console.print()
    ...
    # Package version
    from routesmit import __version__
    console.print(f"  routesmit: {__version__}")
...
def _run_install(target: str) -> None:
    ...
    if result.success:
        console.print(f"[green]✓ Installed routesmit for {target}[/green]")
...
def history(
    ...
    from routesmit.state import list_routes

    config = load_config()
    routes = list_routes(config.routes_dir)[:limit]

    if not routes:
        console.print("[dim]No route history found. Enable with --save-routes or save_routes = true in .routesmit.toml[/dim]")
        return
